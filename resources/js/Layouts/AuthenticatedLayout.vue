...
const systemLogoUrl = computed(() => {
    const safe = String(logoUrl.value ?? '').trim();
    return safe || '';
});
            ...
            <header class="sticky top-0 z-40 flex items-center justify-between border-b border-white/60 bg-white/90 px-4 py-3 shadow-sm backdrop-blur">
                <Link :href="safeRoute('home', '/home')" class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-md bg-slate-900 text-base font-semibold text-white">
                        <img v-if="systemLogoUrl" :src="systemLogoUrl" :alt="systemBrandName" class="h-6 w-6 object-contain" />
                        <span v-else class="text-xs font-bold tracking-[0.08em] text-white">{{ systemBrandInitials }}</span>
                    </div>
        ...
        <template v-else>
            <div class="flex min-h-screen">
                ...
                <div class="flex flex-1 flex-col">
                    ...
                    <nav class="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200/80 bg-white/95 px-2 pt-2 pb-[max(env(safe-area-inset-bottom),0.45rem)] shadow-[0_-10px_30px_-20px_rgba(15,23,42,0.6)] backdrop-blur md:hidden">
                        <div class="mx-auto flex max-w-lg items-end gap-1">
                            <button
                                type="button"
                                class="flex min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-slate-700"
                            ...
                            <Link
                                ...
                                :class="isLinkActive(link) ? 'text-white shadow-inner shadow-black/10' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'"
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-md bg-slate-900 text-xs font-semibold text-white">
                                            <img v-if="systemLogoUrl" :src="systemLogoUrl" :alt="systemBrandName" class="h-5 w-5 object-contain" />
                                            <span v-else class="text-[10px] font-bold tracking-[0.08em] text-white">{{ systemBrandInitials }}</span>
                                        </div>
                                        <div class="flex flex-col leading-tight">
                                            <span class="text-sm font-semibold text-slate-900">{{ systemBrandName }}</span>
                                            <span class="mt-1 inline-flex w-fit items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                                    ...
                                    <button type="button" class="rounded-full bg-slate-100 p-2" @click="closeSidebar" title="Fechar">
                                        <X class="h-4 w-4" />
                                    </button>
                                </div>
                            </div>
