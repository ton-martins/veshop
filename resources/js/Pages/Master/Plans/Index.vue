...
import { Briefcase, ChevronDown, ChevronUp, CircleCheckBig, Eye, Plus, Save, Store, Trash2 } from 'lucide-vue-next';
...
const normalizePlan = (plan, index) => {
    ...
    return {
        id: plan?.id ?? null,
        temp_id: plan?.id ? `plan-${plan.id}` : tempId('draft'),
        isDraft: !plan?.id,
        niche,
...
const plansState = ref((props.plans?.data ?? []).map(normalizePlan));

watch(
    () => props.plans?.data,
    (next) => { plansState.value = (next ?? []).map(normalizePlan); },
    { deep: true },
);
...
const stats = computed(() => {
    const rows = plansState.value.filter((p) => !p.isDraft);
    return {
        total: rows.length,
...
const visiblePlans = computed(() => {
    return plansState.value
        .filter((p) => (!activeNicheTab.value || p.niche === activeNicheTab.value))
        .filter((p) => p.isDraft || statusFilter.value === 'all' || (statusFilter.value === 'active' ? p.is_active : !p.is_active))
        .sort((a, b) => (Number(a.tier_rank ?? 0) - Number(b.tier_rank ?? 0)) || String(a.name).localeCompare(String(b.name)));
});
...
const savePlan = (plan) => {
    ...
    const options = {
        preserveScroll: true,
        onError: (errors) => setErrors(plan, errors),
        onSuccess: () => clearErrors(plan),
        onFinish: () => {
            savingTempId.value = '';
...
const removePlan = (plan) => {
    ...
    if (plan.active_contractors_count > 0) {
        window.alert('Este plano possui contratantes vinculados e não pode ser removido.');
        return;
    }
        ...
        <section class="space-y-4">
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
                <div class="grid gap-3 sm:grid-cols-3">
                    <button type="button" class="rounded-xl border px-3 py-2 text-left" :class="statusFilter === 'all' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200'" @click="statusFilter = 'all'">
                        <p class="text-[11px] uppercase">Cadastrados</p><p class="text-lg font-semibold">{{ stats.total }}</p>
                    </button>
                    <button type="button" class="rounded-xl border px-3 py-2 text-left" :class="statusFilter === 'active' ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-slate-200'" @click="statusFilter = 'active'">
                        <p class="text-[11px] uppercase">Ativos</p><p class="text-lg font-semibold">{{ stats.active }}</p>
                    </button>
                    <button type="button" class="rounded-xl border px-3 py-2 text-left" :class="statusFilter === 'inactive' ? 'border-rose-600 bg-rose-600 text-white' : 'border-slate-200'" @click="statusFilter = 'inactive'">
                        <p class="text-[11px] uppercase">Inativos</p><p class="text-lg font-semibold">{{ stats.inactive }}</p>
                    </button>
                </div>

                <div class="mt-4 flex flex-wrap items-center gap-2">
                    <button type="button" class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white" @click="addDraft">
                    ...
                    <button
                        ...
                        :class="activeNicheTab === tab.value ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 text-slate-700'"
                ...
                <div class="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    <article v-for="plan in visiblePlans" :key="plan.temp_id" class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm" :class="plan.is_featured ? 'ring-1 ring-emerald-200' : ''">
                        ...
                        <div class="mt-3 grid gap-2">
                            ...
                            <button type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60" :disabled="deleteForm.processing || deletingId === plan.id" @click="removePlan(plan)">
