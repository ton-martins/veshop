...
class MasterUserService
{
     ...
     */
    public function store(array $data, ?UploadedFile $avatarFile): RedirectResponse
    {
        ...
        $this->syncUserContractors($user, is_array($contractorIds) ? $contractorIds : []);
     ...
     * @param  array<string, mixed>  $data
     */
    public function update(Request $request, User $user, array $data, ?UploadedFile $avatarFile): RedirectResponse
    {
        $contractorIds = $data['contractor_ids'] ?? [];
        unset($data['contractor_ids'], $data['avatar']);
