...
class MasterContractorService
{
        ...
        $contractors = $query
            ...
            ->through(function (Contractor $contractor): array {
                ...
                return [
                    ...
                    'contract_starts_at' => optional($contractor->contract_starts_at)?->format('Y-m-d'),
                    'contract_ends_at' => optional($contractor->contract_ends_at)?->format('Y-m-d'),
     ...
     */
    public function update(array $data, Contractor $contractor): RedirectResponse
    {
        $slugBase = $data['slug'] ?: $data['name'];
        $niche = $this->normalizeNiche($data['business_niche']);
        $businessType = Contractor::normalizeBusinessType((string) ($data['business_type'] ?? ''), $niche);
